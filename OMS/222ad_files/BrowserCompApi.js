/*
* NEW BrowserCompAPI version 1.0.0.35
...
if (!isNotSmartbar() && (/___requireStorage/.test(document.location.href) || myconduitAppData || typeof (abstractionlayer) !== "undefined")) {
...
conduitAppData = typeof(localStorage) !== "undefined" ? localStorage.getItem("smartbarBcApi") : undefined;
...
if (conduitAppData)
conduit.currentApp = JSON.parse(conduitAppData);
}
if (conduit.currentApp)
msgSender = JSON.stringify({ appId: conduit.currentApp.appId, context: conduit.currentApp.context, viewId: conduit.currentApp.viewId, popupId: conduit.currentApp.popupId, menuId: conduit.currentApp.menuId });
}
...
var addListenerCallback = function (request) {
if (request.data && request.data.action === "topicGetMessage") {
...
dest = "webappApi";
data = JSON.stringify({ method: targetMethod, params: params });
}
if (dest) {
...
conduit.app.popup.resize(null,{ height: fixHeight(parseInt(newHeight, 10)), width: -1 });
}
};
this.changeSize = function (newWidth, newHeight) {
if (conduit.currentApp.context === "popup") {
conduit.app.popup.resize(null,{ width: parseInt(newWidth, 10), height: fixHeight(parseInt(newHeight, 10)) });
}
};
this.changeWidth = function (newWidth) {
...
: document.documentElement ? document.documentElement.offsetHeight : undefined;
};
this.getInfo = function () {
...
this.getMainFrameTitle = function () {
return data.currentTab.title;
};
this.getMainFrameUrl = function () { return data.currentTab.url; };
...
conduit.tabs.executeScript(tabId || null, { code: appendToolbarApi });
conduit.tabs.executeScript(tabId || null, { code: "function EBCallBackMessageReceived(data){ }" });
}
...
this.openGadget3 = function (url, width, height, features, isComingFromOpenGadget2) {
...
if (isComingFromOpenGadget2){
if (navigator.userAgent.indexOf("Firefox") != -1) {
if(!featuresObj.titlebar){
height += 35;
}
}
}
...
allowScrolls: (featuresObj.hscroll === false && featuresObj.vscroll === false) ? false : true
});
};
this.openGadget2 = function (url, width, height, features) {
this.openGadget3(url, width, fixHeight(height), features,  true);
};
